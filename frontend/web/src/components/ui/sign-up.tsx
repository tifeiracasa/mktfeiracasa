...
interface SignUpPageProps {
  ...
  onSignUp?: (data: CustomerData, password: string) => void;
  onGoogleSignUp?: () => void;
  onSignIn?: () => void;
  step?: 'account' | 'personal' | 'address';
}
...
const ProgressBar = ({ currentStep }: { currentStep: 'account' | 'personal' | 'address' }) => {
...
const PersonalStep = ({ formData, setFormData, onNext, onPrev }: any) => (
  <>
    ...
    <div className="flex gap-4">
      <button 
        type="button"
        onClick={onPrev}
        className="animate-element animate-delay-700 flex-1 rounded-2xl border border-border py-4 font-medium hover:bg-secondary transition-colors"
      >
        Back
      </button>
      <button 
        type="button"
        onClick={onNext}
        className="animate-element animate-delay-700 flex-1 rounded-2xl bg-primary py-4 font-medium text-primary-foreground hover:bg-primary/90 transition-colors"
      >
        Continue
      </button>
    </div>
...
const AddressStep = ({ formData, setFormData, onSubmit, onPrev }: any) => (
  <>
    <div className="animate-element animate-delay-300">
    ...
    <div className="grid grid-cols-3 gap-4">
      <div className="col-span-2 animate-element animate-delay-400">
    ...
    <div className="animate-element animate-delay-500">
      <label className="text-sm font-medium text-muted-foreground">Complement (Optional)</label>
      <GlassInputWrapper>
        <input 
          name="complement"
          type="text"
    ...
    <div className="flex gap-4">
      <button 
        type="button"
        onClick={onPrev}
        className="animate-element animate-delay-800 flex-1 rounded-2xl border border-border py-4 font-medium hover:bg-secondary transition-colors"
      >
        Back
      </button>
      <button 
        type="submit"
        onClick={onSubmit}
        className="animate-element animate-delay-800 flex-1 rounded-2xl bg-primary py-4 font-medium text-primary-foreground hover:bg-primary/90 transition-colors"
      >
        Create Account
      </button>
    </div>
...
export const SignUpPage: React.FC<SignUpPageProps> = ({
  ...
  description = "Join our platform and start your journey with us",
  heroImageSrc,
  testimonials = [],
  onSignUp,
  onGoogleSignUp,
  onSignIn,
  step: initialStep = 'account'
}) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const { password, confirmPassword, ...customerData } = formData;
    onSignUp?.(customerData as CustomerData, password);
  };
    ...
    <div className="h-[100dvh] flex flex-col md:flex-row font-geist w-[100dvw]">
      ...
      <section className="flex-1 flex items-center justify-center p-8">
        <div className="w-full max-w-md">
          <div className="flex flex-col gap-6">
            ...
            <h2 className="text-xl font-medium text-center">{getStepTitle()}</h2>
      ...
      {heroImageSrc && (
        <section className="hidden md:block flex-1 relative p-4">
          <div className="animate-slide-right animate-delay-300 absolute inset-4 rounded-3xl bg-cover bg-center bg-[url('https://images.unsplash.com/photo-1559136555-9303baea8ebd?w=2160&q=80')]"></div>
